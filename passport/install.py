...
def install(root, ref, dry_run=False, remove=False):
    ...
    if remove:
        for name in ("AGENTS.md", "CLAUDE.md"):
            generated = safe_file(root, name)
            if generated.exists():
                text = generated.read_text()
                if "GENERATED BY" in text and ".passport/POLICY.md" in text:
                    raise Invalid("Remove Passport local context through the owning generator before removal: "+name)
    changes = {}
    new = {"files": {}, "blocks": {}}
    ...
    for name, text in blocks.items():
        ...
        if START in before or END in before:
            ...
            if "GENERATED BY" in before:
                if ".passport/POLICY.md" in before and not remove:
                    continue
                raise Invalid("Generated instructions preserved. Add .passport/POLICY.md through the owning generator, then retry: "+name)
